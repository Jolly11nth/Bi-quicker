...
RIDERS = [{"email": "rider@example.com", "name": "Demo Rider"}, {"email": "rider2@example.com", "name": "Samuel Rider"}, {"email": "rider3@example.com", "name": "Daniel Rider"}]
...
def route_between(db: Session, customer_email: str, vendor_owner_email: str) -> tuple[float, float]:
    ...
    if customer is None or vendor is None: raise HTTPException(status_code=400, detail="Delivery route is unavailable. Customer and vendor location access is required before checkout.")
    coordinates = f"{vendor.longitude},{vendor.latitude};{customer.longitude},{customer.latitude}"
    try:
        response = httpx.get(f"https://router.project-osrm.org/route/v1/driving/{coordinates}?overview=false", timeout=15); response.raise_for_status(); data = response.json(); route = data.get("routes", [None])[0]
    except Exception as exc: raise HTTPException(status_code=502, detail="Routing service is temporarily unavailable. Please try again.") from exc
    if data.get("code") != "Ok" or not route: raise HTTPException(status_code=404, detail="No drivable route found between the customer and vendor.")
...
def signup(payload: SignupPayload):
    ...
    with Session(engine) as db:
        ...
        db.add(User(email=email, name=name, role=payload.role, password_hash=hash_password(payload.password), profile_data=json.dumps(payload.data))); db.commit()
...
def signin(payload: AuthPayload):
    ...
    if demo and email == demo[0] and payload.password == "demo123": return {"token": create_session(email, payload.role, demo[1]), "session": {"role": payload.role, "email": email, "name": demo[1]}}
    with Session(engine) as db:
...
def save_location(payload: LocationPayload, authorization: str | None = Header(default=None)):
    session = current_session(authorization); validate_role(payload.role); email = normalize_email(payload.email)
    if session["email"] != email or session["role"] != payload.role: raise HTTPException(status_code=403, detail="You can only save your own location")
    with Session(engine) as db:
...
def get_location(role: str, email: str, authorization: str | None = Header(default=None)):
    session = current_session(authorization); validate_role(role); normalized = normalize_email(email)
    if session["role"] != "admin" and session["email"] != normalized: raise HTTPException(status_code=403, detail="You can only access your own location")
...
def create_order(payload: OrderCreatePayload, authorization: str | None = Header(default=None)):
    ...
    if session["role"] != "customer": raise HTTPException(status_code=403, detail="Only customers can create orders")
    with Session(engine) as db:
        vendor = db.get(VendorRecord, payload.vendorId)
        if vendor is None: raise HTTPException(status_code=404, detail="Vendor not found")
        raw_products = load(vendor.products_json, [])
        products = {item["id"]: item for item in raw_products if isinstance(item, dict)} if isinstance(raw_products, list) else {}
        items: list[dict] = []; subtotal = 0.0
        for requested in payload.items:
            product = products.get(requested.productId)
            if product is None: raise HTTPException(status_code=400, detail=f"Product {requested.productId} is no longer available")
            if requested.quantity > product["stock"]: raise HTTPException(status_code=400, detail=f"Only {product['stock']} units of {product['name']} are available")
            line = {"productId": product["id"], "name": product["name"], "price": product["price"], "quantity": requested.quantity}; items.append(line); subtotal += product["price"] * requested.quantity
        distance_km, duration_minutes = route_between(db, normalize_email(session["email"]), vendor.owner_email)
        try: delivery_fee = rider_fee(distance_km)
        except ValueError as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc
        maintenance = maintenance_fee(subtotal); total = subtotal + maintenance + delivery_fee; order_id = f"BQ-{secrets.token_hex(4).upper()}"; created_at = utcnow(); payment_reference = f"BQ-{secrets.token_hex(6).upper()}"
        payment = {"provider": "paystack", "reference": payment_reference, "status": "unpaid", "confirmed": False, "currency": "NGN", "amount": total, "authorizationUrl": ""}
        participants = [{"role": "customer", "email": session["email"], "name": session["name"], "active": True}, {"role": "store", "email": vendor.owner_email, "name": vendor.name, "active": True}]
        ...
        row = OrderRecord(id=order_id, customer_email=session["email"], customer_name=session["name"], vendor_id=vendor.id, vendor_name=vendor.name, vendor_owner_email=vendor.owner_email, items_json=dump(items), subtotal=subtotal, maintenance_fee=maintenance, maintenance_rate=MAINTENANCE_RATE, delivery_distance_km=distance_km, delivery_minutes=duration_minutes, delivery_fee=delivery_fee, total=total, status="Awaiting payment", payment_json=dump(payment), participants_json=dump(participants), messages_json=dump(messages), tracking_json=dump(tracking_events(created_at)))
        db.add(row); db.commit(); db.refresh(row); return order_dict(row)
...
def order(order_id: str, authorization: str | None = Header(default=None)):
    ...
    with Session(engine) as db:
        ...
        if not (session["role"] == "admin" or session["email"] in {row.customer_email, row.vendor_owner_email, row.rider_email}): raise HTTPException(status_code=403, detail="You do not have access to this order")
...
def add_message(order_id: str, payload: MessagePayload, authorization: str | None = Header(default=None)):
    ...
    with Session(engine) as db:
        row = db.get(OrderRecord, order_id)
        if row is None: raise HTTPException(status_code=404, detail="Order not found")
        participants = load(row.participants_json, []); allowed = session["role"] == "admin" or any(p.get("email") == session["email"] and p.get("active") for p in participants if isinstance(p, dict))
        if not allowed: raise HTTPException(status_code=403, detail="You are not a participant in this order")
        messages = load(row.messages_json, []); messages.append({"id": f"{order_id}-m-{secrets.token_hex(4)}", "senderRole": session["role"], "senderEmail": session["email"], "senderName": session["name"], "body": payload.body.strip(), "at": utcnow()}); row.messages_json = dump(messages); db.commit(); return order_dict(row)
...
def advance_order(order_id: str, authorization: str | None = Header(default=None)):
    ...
    with Session(engine) as db:
        ...
        if row.status in {"Awaiting payment", "Delivered"}: raise HTTPException(status_code=400, detail="This order cannot be advanced in its current state")
        ...
        next_status = ORDER_SEQUENCE[current + 1]; at = utcnow()
        if next_status == "Rider assigned":
            rider = RIDERS[0]; row.rider_email, row.rider_name = rider["email"], rider["name"]; participants = load(row.participants_json, []); participants.append({"role": "rider", "email": rider["email"], "name": rider["name"], "active": True}); row.participants_json = dump(participants); messages = load(row.messages_json, []); messages.append({"id": f"{order_id}-rider-{secrets.token_hex(4)}", "senderRole": "system", "senderEmail": "system", "senderName": "Bi-quicker", "body": f"{rider['name']} has been assigned to this delivery.", "at": at, "system": True}); row.messages_json = dump(messages)
        row.status = next_status; tracking = load(row.tracking_json, []); target = ORDER_SEQUENCE.index(next_status)
        for index, event in enumerate(tracking):
            if index <= target:
                event["done"] = True
                if not event.get("at"): event["at"] = at
        row.tracking_json = dump(tracking); db.commit(); return order_dict(row)
...
def initialize_payment(order_id: str, authorization: str | None = Header(default=None)):
    ...
    with Session(engine) as db:
        ...
        try: data = initialize_transaction(email=row.customer_email, amount_naira=row.total, reference=payment["reference"], callback_url=os.environ.get("PAYSTACK_CALLBACK_URL") or None, metadata={"order_id": row.id, "vendor_id": row.vendor_id})
        except (RuntimeError, ValueError) as exc: raise HTTPException(status_code=503, detail=str(exc)) from exc
        payment.update({"status": "initialized", "authorizationUrl": data["authorization_url"], "accessCode": data["access_code"]}); row.payment_json = dump(payment); db.commit(); return payment
...
def road_route(payload: dict):
    ...
    try: response = httpx.get(f"https://router.project-osrm.org/route/v1/driving/{coordinates}?overview=false", timeout=15); response.raise_for_status(); data = response.json(); route = data.get("routes", [None])[0]
