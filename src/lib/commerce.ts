...
export interface ChatMessage {
  id: string
  senderRole: CommerceRole | 'system'
  senderEmail: string
  senderName: string
  body: string
  at: string
...
const seedVendors: Vendor[] = [
  {
    id: 'vendor-fresh-basket', name: 'Fresh Basket', category: 'Groceries', rating: '4.8', eta: '20–35 min', ownerEmail: 'store@example.com',
    paymentBank: 'GTBank', paymentAccountName: 'Fresh Basket Ltd', paymentAccountNumber: '0123456789',
    products: [
  ...
  {
    id: 'vendor-tech-hub', name: 'Tech Hub', category: 'Electronics', rating: '4.7', eta: '25–40 min', ownerEmail: 'techhub@example.com',
    paymentBank: 'Access Bank', paymentAccountName: 'Tech Hub NG', paymentAccountNumber: '1029384756',
    products: [
  ...
  {
    id: 'vendor-home-store', name: 'Home Store', category: 'Home & Kitchen', rating: '4.6', eta: '30–45 min', ownerEmail: 'homestore@example.com',
    paymentBank: 'UBA', paymentAccountName: 'Home Store NG', paymentAccountNumber: '2019283746',
    products: [
  ...
  {
    id: 'vendor-market-square', name: 'Market Square', category: 'General', rating: '4.5', eta: '25–40 min', ownerEmail: 'marketsquare@example.com',
    paymentBank: 'First Bank', paymentAccountName: 'Market Square Stores', paymentAccountNumber: '3018273645',
    products: [
...
export const getVendors = (): Vendor[] => {
  const vendors = read<Vendor[]>(VENDORS_KEY, [])
  if (vendors.length) return vendors
  write(VENDORS_KEY, seedVendors)
  return seedVendors
}
...
export const createOrder = (input: {
  ...
  const order: CommerceOrder = {
    ...
    createdAt,
    payment: {
      bank: input.vendor.paymentBank,
      accountName: input.vendor.paymentAccountName,
      accountNumber: input.vendor.paymentAccountNumber,
      reference: `BQ-${Date.now().toString().slice(-6)}`,
      confirmed: false,
    },
    participants: [
      { role: 'customer', email: input.customerEmail, name: input.customerName, active: true },
      { role: 'store', email: input.vendor.ownerEmail, name: input.vendor.name, active: true },
    ],
    messages: [{
...
export const updateOrder = (id: string, patch: Partial<CommerceOrder>) => {
...
export const confirmPayment = (id: string) => {
  ...
  const at = now()
  const tracking = order.tracking.map((event, index) => index <= 1 ? { ...event, done: true, at: index === 1 ? at : event.at } : event)
  return updateOrder(id, { status: 'Paid', payment: { ...order.payment, confirmed: true }, tracking })
}

export const addOrderMessage = (id: string, message: Omit<ChatMessage, 'id' | 'at'>) => {
  const order = getOrder(id)
  if (!order) return null
  const next = { ...message, id: `${id}-m-${Date.now()}`, at: now() }
  return updateOrder(id, { messages: [...order.messages, next] })
}

export const assignRider = (id: string, riderEmail: string) => {
  const order = getOrder(id)
  const rider = riders.find((item) => item.email === riderEmail)
  if (!order || !rider) return null
  const at = now()
  const participants = order.participants.some((item) => item.email === rider.email)
    ? order.participants
    : [...order.participants, { role: 'rider' as const, email: rider.email, name: rider.name, active: true }]
  const tracking = order.tracking.map((event) => event.id === 'assigned' ? { ...event, done: true, at } : event)
  const systemMessage: ChatMessage = {
    id: `${id}-rider-${Date.now()}`, senderRole: 'system', senderEmail: 'system', senderName: 'Bi-quicker',
    body: `${rider.name} has been assigned to this delivery and added to the order chat.`, at, system: true,
  }
  return updateOrder(id, { riderEmail: rider.email, riderName: rider.name, riderAssignedAt: at, status: 'Rider assigned', participants, messages: [...order.messages, systemMessage], tracking })
}

export const advanceOrder = (id: string) => {
  const order = getOrder(id)
  if (!order) return null
  const sequence: OrderStatus[] = ['Awaiting payment', 'Paid', 'Preparing', 'Rider assigned', 'Picked up', 'In transit', 'Delivered']
  const current = sequence.indexOf(order.status)
  const nextStatus = sequence[Math.min(current + 1, sequence.length - 1)]
  const at = now()
  const tracking = order.tracking.map((event, index) => {
    const targetIndex = sequence.indexOf(nextStatus)
    return index <= targetIndex ? { ...event, done: true, at: event.at || at } : event
  })
  return updateOrder(id, { status: nextStatus, tracking })
}
